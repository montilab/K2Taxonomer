...
colnames(enrTable) <- c("Gene Set", "Node", "Edge", "Direction", "P Value_Hyper", "FDR_Hyper", "N_Overlap", "N_Sig. Genes", "N_Gene Set", "P Value_ssGSEA", "FDR_ssGSEA", "Diff_ssGSEA", "Mean_ssGSEA", "Hits", "Plot", "Send", "Link")
...
dgeTable <- dgeTable[,c("gene", "node", "edge", "direction", "pval", "fdr", "coef", "mean", "Plot", "Send", "Link")]
...
## Rename columns
colnames(dgeTable) <- c("Gene", "Node", "Edge", "Direction", "P Value", "FDR", "Diff", "Mean", "Plot", "Send", "Link")
...
output$Match <- renderUI({
  ...
  if(!is.null(input$mstring) && (input$mstring != "" & sum(grepl(input$mstring, infoMat, ignore.case = TRUE)) != 0)) {

      ## Find matches in the terminal leafs
      infoInd <- unique(which(`dim<-`(grepl(input$mstring, infoMat, ignore.case = TRUE), dim(infoMat)), arr.ind=TRUE)[,1])
...
output$heatmapPlot <- renderPlotly({
  ...
  if(values$nodeSel %in% names(K2res)){
    ...
    if(!is.null(values$selCov)){
      ...
      names(colSidePalette) <- c(colValues, "Edge:1", "Edge:2", paste0(values$selCov, ":NA"))
...
output$stabStats <- renderUI({
  if(values$nodeSel %in% names(K2res)){
    ...
    outNode <- paste0("<br> Partition Stability: <b>", signif(nodeStab, 2), "</b> <br>")
    outStab <- paste0("Subgroup Stability <br> &emsp; Edge:1= <b>", signif(sampStab[1], 2), "</b> &emsp; Edge:2= <b>",  signif(sampStab[2], 2))
...
if (!is.null(K2res[[1]]$modTests)) {
  ...
  ## Get child
  K2modTestFram$Child <- apply(K2modTestFram[, c("node", "edge")], 1, function(x){
  ...
  output$metaVarTab <- renderDataTable({
    ...
    K2modTestFram <- K2modTestFram[, c("Subgroup", "Child", "value", "pval", "fdr")]
    colnames(K2modTestFram) <- c("Subgroup", "Child", "Variable", "P Value", "Q Value")
    ...
    datatable(K2modTestFram,
              ...
              options = list(dom = 'Brt',
                             ...
                             scrollY = "400px",
                             paging = FALSE,
                             pageLength = nrow(K2modTestFram),  # Set because paging option is ignored on some systems
  ...
  output$metaVarTab <- renderDataTable({
    ...
    ## Set null data table
    values$mvTab <- NULL
    K2modTestFramNULL <- data.frame("No phenotypic variable results."); colnames(K2modTestFramNULL) <- NULL
    datatable(K2modTestFramNULL,
              ...
              options = list(dom = 'Brt',
                             ...
                             scrollY = "400px",
                             paging = FALSE,
                             pageLength = nrow(K2modTestFramNULL), # Set because paging option is ignored on some systems
                             buttons = c('csv', 'excel'),
                             columnDefs = list(list(className = 'dt-center', targets = "_all"))),
              selection = "single")
...
if (!is.null(K2res[[1]]$modTests)) {
  ...
  phenoHelpShow <- function() {
    div(id = "phenoHelp",
              modalDialog("1) Select row to visualize results in dendrogram.",
                    br(),
                    tags$div("2) Download table to view full set of statistical values.
                    For description of this table go",
                    tags$a(href = 'https://montilab.github.io/K2Taxonomer/reference/getTestsModTable.html','here'), 
                    "."),
                    br(),
  ...
  phenoTabDL <- function() {
    div(id = "phenoDL",
        modalDialog(downloadButton("downloadPhenoCSV", "Download Table as CSV file"),
                    br(),
                    br(),
  ...
  if(values$nodeSel != "No Selection"){
    ...
    datatable(infoSub,
              ...
              options = list(dom = 'Brt',
                             ...
                             scrollY = "400px",
                             paging = FALSE,
                             pageLength = nrow(infoSub), # Set because paging option is ignored on some systems
                             buttons = c('csv', 'excel'),
                             columnDefs = list(
                 ...
                 if (grepl("PlotRow|SendRow", dgeVal)) {
                   ...
                   if (grepl("PlotRow", dgeVal)) {
                     ...
                     if (values$Gene %in% names(gene2Pathway)) {
                       values$dgeHits <- paste0("^", gsub("; ", "$|^", gene2Pathway[[values$Gene]]), "$")
                     } else {
               ...
               if (!is.null(input$HE_cell_clicked$value)) {
                 ...
                 if (grepl("PlotRow|SendRow", hyperVal)) {
                   ...
                   if (grepl("PlotRow", hyperVal)) {
                     ...
                     values$geneList <- paste(paste0("^", genesets[[values$GeneSet]], "$"), collapse = "|")
                   }
                 }
               }
             })
...
### Click and hold nodes to select subgroups (Click whitespace to reset selections)
...
observeEvent(input$compareGo, {
  ...
  withProgress(message = "Comparing Selected Subgroups:", value = 0, {
    ...
    clusterRes$Subgroup <- names(values$groupListClicked)[as.numeric(clusterRes$edge)]
    ...
    ssEnrRes$Subgroup <- names(values$groupListClicked)[as.numeric(ssEnrRes$edge)]
    ...
    ## Sort columns
    EnrRes <- EnrRes[, c("category", "Subgroup", "pval_hyper", "fdr_hyper", "nhits", "ndrawn", "ncats", "pval_limma", "fdr_limma", "coef", "mean", "hits")]
    ...
    ## Change column names
    colnames(EnrRes)  <- c("Gene Set", "Subgroup", "P Value_Hyper", "FDR_Hyper", "N_Overlap", "N_Sig. Genes", "N_Gene Set", "P Value_ssGSEA", "FDR_ssGSEA", "Diff_ssGSEA", "Mean_ssGSEA", "Hits", "Plot", "Send", "Link")
    ...
    clusterRes <- clusterRes[,c("gene", "Subgroup", "pval", "fdr",  "coef", "mean", "Plot", "Send", "Link")]
    ...
    ## Rename columns
    colnames(clusterRes) <- c("Gene", "Subgroup", "P Value", "FDR", "Diff", "Mean", "Plot", "Send", "Link")
...
observeEvent(input$DGEmulti_search_columns, {
  if (!is.null(values$geneListMulti)) {
    if(input$DGEmulti_search_columns[1] != values$geneListMulti) {
      ...
      values$nodeSelHEMulti <- gsub("\\$|\\^", "", input$DGEmulti_search_columns[2])
    }
  }
})
                 ...
                 if (grepl("PlotRow|SendRow", dgeVal)) {
                   ...
                   if (grepl("PlotRow", dgeVal)) {
                     ...
                     if (values$GeneMulti %in% names(gene2Pathway)) {
                       values$dgeHitsMulti <- paste0("^", gsub("; ", "$|^", gene2Pathway[[values$GeneMulti]]), "$")
                     } else {
...
observeEvent(input$HEmulti_search_columns, {
  if (!is.null(values$dgeHitsMulti)) {
    if(input$HEmulti_search_columns[1] != values$dgeHitsMulti) {
      ...
      values$nodeSelDGEMulti <- gsub("\\$|\\^", "", input$HEmulti_search_columns[2])
    }
  }
})
               ...
               if (!is.null(input$HEmulti_cell_clicked$value)) {
                 ...
                 if (grepl("PlotRow|SendRow", hyperVal)) {
                   ...
                   if (grepl("PlotRow", hyperVal)) {
                     ...
                     values$geneListMulti <- paste(paste0("^", genesets[[values$GeneSetMulti]], "$"), collapse = "|")
                   }
                 }
               }
             })
