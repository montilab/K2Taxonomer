...
output$Match <- renderUI({
  ...
  if(!is.null(input$mstring) && (input$mstring != "" & sum(grepl(input$mstring,
        infoMat, ignore.case=TRUE)) != 0)) {
...
if (!is.null(K2res[[1]]$modTests)) {
  ...
  output$metaVarTab <- renderDataTable({
    ...
    K2modTestFram <- K2modTestFram[, c("Subgroup", "Child", "value", "pval",
        "fdr")]
    colnames(K2modTestFram) <- c("Subgroup", "Child", "Variable", "P Value",
        "Q Value")
...
columns",
                  br(),
                  HTML("2) Select '&#128202;' to plot single-sample enrichment
below."),
                  br(),
                  HTML("3) Select '&#9992;' to send row information to look up
...
observeEvent(input$Held, {
  ...
  if (!is.null(curVal)) {
    ...
    if(curVal %in% names(K2res)) {
      ...
      obs <- strsplit(as.character(vNetOut$x$nodes$label[vNetOut$x$nodes$id ==
          curVal]), "\n")[[1]]
    }
...
observeEvent(input$compareGo, {
  ...
  withProgress(message="Comparing Selected Subgroups:", value=0, {
    ...
    colnames(hyperEnrRes)[colnames(hyperEnrRes) %in% c("pval", "fdr")] <-
      paste(colnames(hyperEnrRes)[colnames(hyperEnrRes) %in% c("pval", "fdr")],
        "hyper", sep="_")
    ...
    EnrRes <- EnrRes[, c("category", "Subgroup", "pval_hyper", "fdr_hyper",
        "nhits", "ndrawn", "ncats", "pval_limma", "fdr_limma", "coef", "mean",
        "hits")]
    ...
    ## Change column names
    colnames(EnrRes)  <- c("Gene Set", "Subgroup", "P Value_Hyper", "FDR_Hyper",
        "N_Overlap", "N_Sig. Genes", "N_Gene Set", "P Value_ssGSEA",
        "FDR_ssGSEA", "Diff_ssGSEA", "Mean_ssGSEA", "Hits", "Plot",
        "Send", "Link")
...
geneTabDLMulti <- function() {
  div(id="geneDLMulti",
      modalDialog(downloadButton("downloadGeneCSVMulti", "Download Table as CSV
        file"),
                  br(),
                  br(),
                  easyClose=TRUE, title="Download Table")
