...
output$dendro <- renderVisNetwork({
  ...
  if(!is.null(values$mvTabSub)) {
    if(nrow(values$mvTabSub) > 0) {
      ...
      mEdge <- values$mvTabSub[, c("split", "Child", "width")]
...
### Subgroup Information (&uarr; = Group 1; &darr; = Group 2)
...
output$stabStats <- renderUI({
  if(values$nodeSel %in% names(K2res)){
    ...
    outStab <- paste0("Subgroup Stability <br> &emsp; Group 1: <b>", signif(sampStab[1], 2), "</b> &emsp; Group 2: <b>",  signif(sampStab[2], 2))
...
if (!is.null(K2res[[1]]$modTests)) {
  ...
  K2modTestFram <- modTestsTable[,c("split", "mod", "value", "pval", "fdr")]

  ## Get child
  K2modTestFram$Child <- apply(K2modTestFram[, c("split", "mod")], 1, function(x){
    vSub <- vNetOut$x$edges[vNetOut$x$edges$from == x[1],]
    vSub$to[as.numeric(x[2])]
  })

  ## Get split
  K2modTestFram$Split <- paste0(K2modTestFram$split, K2modTestFram$mod)
  ...
  phenoHelpShow <- function() {
    div(id = "phenoHelp",
              modalDialog("1) Select row to visualize results in dendrogram.",
                    br(),
                    "2) Download table to view full set of statistical values.
                    For description of this table go 
                    <a href='https://montilab.github.io/K2Taxonomer/reference/getTestsModTable.html' 
                    >here</a>.",
                    br(),
