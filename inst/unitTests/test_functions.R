test_K2Tworkflow <- function() {
    ...
    checkTrue(is(dend, "dendrogram"), "K2dendro() did not return 'dendrogram' object.\n")
    ...
    checkTrue(is(mvRes, "data.frame"), "getTestsModTable() did not return 'data.frame'.\n")
    ...
    checkTrue(is(gRes, "data.frame"), "getDGETable() did not return 'data.frame'.\n")
    ...
    checkTrue(is(gp, "character"), "runGSEmods() did not return 'character' vector.\n")
    ...
    checkTrue(is(K2check, "K2"), "aggregateGSVAscores() did not return 'K2' object.\n")
    ...
    checkTrue(is(eRes, "data.frame"), "getEnrichmentTable() did not return 'data.frame'.\n")

    ## Dashboard
    K2dashboard(K2res, output_dir = tempdir())
    K2resDir <- list.files(tempdir(), pattern = "K2Taxonomer_", full.names = TRUE)[[1]]
    checkTrue(file.exists(file.path(K2resDir, "about.md")), "K2dashboard() didn't write files.")
    checkTrue(file.exists(file.path(K2resDir, "K2results.rds")), "K2dashboard() didn't write files.")
    checkTrue(file.exists(file.path(K2resDir, "K2Taxonomer.Rmd")), "K2dashboard() didn't write files.")
    ...
    checkTrue(is(K2check, "K2"), "runK2Taxonomer() did not return 'K2' object.\n")
