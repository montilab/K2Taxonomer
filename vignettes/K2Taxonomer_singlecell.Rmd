...
package: K2Taxonomer
vignette: >
  %\VignetteIndexEntry{K2Taxonomer_singlecell}
...
For this example we will use a single-cell RNAseq dat set a of 379 cells from the mouse visual cortex which is a subset from  [@tasic2016adult], made available by the `r Biocpkg("scRNAseq")` package.
...
Given that this data set is relatively small, we will need to remove lowly represented cell types. These will include six cell types with fewer than 5 cells. Additionally, we will remove 30 cells without cell type labels (NA).
...
At this point the rest of the analysis identical to that of the basic analysis as presented in the [Basic run of K2Taxonomer](https://montilab.github.io/K2Taxonomer/articles/RunningK2Taxonomer.html) vignette. Please refer to this document for further description of these steps.
