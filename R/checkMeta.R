...
.checkMeta <- function(K2res, arg = NULL, argValue = NULL) {
  ...
  if(!is.null(arg)) {
    ...
    if(!K2m$ssGSEAalg %in% c("ssgsea", "gsva", "zscore", "plage")) {
      stop("Argument, ssGSEAalg, must be one of: 'ssgsea', 'gsva', 'zscore', 'plage'.\n")
    }
    
    # linkage
    if(!K2m$linkage %in% c("mcquitty", "ward.D", "ward.D2", "single", "complete", "average", "centroid")) {
      stop("Argument, linkage, must be one of 'mcquitty', 'ward.D', 'ward.D2', 'single', 'complete', 'average', 'centroid'.\n")
    }
    ...
    if(!is.null(K2m$cohorts)  && !K2m$cohorts %in% colnames(pData(K2eSet(K2res)))) {
      stop("Argument, cohorts, must match column name in phenodata of expression set.\n")
    }
    
    # vehicle
    if(!is.null(K2m$vehicle)  && !K2m$vehicle %in% colnames(pData(K2eSet(K2res)))) {
      stop("Argument, vehicle, must match column name in phenoData of expressionSet.\n")
    }
    
    # block
    if(!is.null(K2m$block)  && !K2m$block %in% colnames(pData(K2eSet(K2res)))) {
      stop("Argument, block, must match column name in phenoData of expressionSet.\n")
    }
