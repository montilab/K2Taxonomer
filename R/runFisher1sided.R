...
.runFisher1sided <- function(csDF) {
    ...
    if (sum(!is.na(csDF$value[csDF$cat])) > 0 & sum(!is.na(csDF$value[!csDF$cat])) > 
        0) {
    ...
    ## Get hits
    hits <- paste(csDF$cohort[csDF$cat & csDF$value == levels(csDF$value)[2] & !is.na(csDF$value)], 
        collapse = ", ")
    ...
    data.frame(pval = pval, stat = stat, df = NA, obsMean = NA, altMean = NA, diffMean = NA, 
        nhits = nhits, ncase = ncase, nalt = nalt, ndrawn = ndrawn, hits = hits, 
