...
#'
#' This function will generate an interactive dashboard of the annotated K=2 clustering results.
#' @param K2results Object of class K2. The output of runK2Taxonomer().
#' @param analysis_name Character string of the name of analysis to write files and generate title of report.
#' @param output_dir Base directory to put report.
#' @param compile Logical. Whether or not to compile dashboard now.
#' @return Nothing.  Writes files to a specified directory.
#' @keywords clustering
#' @export
#' @examples
#' K2dashboard(K2results, analysis_name = "K2Taxonomer", output_dir = ".", compile = TRUE)
#' 

K2dashboard <- function(K2results,
  ...
  # Directory to print
  dirPath <- file.path(output_dir,
                       paste(analysis_name_nospace, gsub("-| |:", "_", Sys.time()), sample(1E7, 1), sep = "_"))
  # RMD file
  ...
  # Compile
  if(compile) rmarkdown::run(RMDpath)
