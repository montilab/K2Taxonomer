...
.runTtest <- function(csDF, alternative = "two.sided") {
    ...
    if (sum(!is.na(csDF$value[csDF$cat])) > 2 & sum(!is.na(csDF$value[!csDF$cat])) > 
        2) {
        TtestOut <- t.test(csDF$value[csDF$cat], csDF$value[!csDF$cat], alternative = alternative)
    ...
    ## Get type of test
    test <- c("one-sided T-test", "two-sided T-test")[as.numeric(alternative == "two.sided") + 
        1]
    
    ## Generate results
    data.frame(pval = pval, stat = stat, df = df, obsMean = obsMean, altMean = altMean, 
        diffMean = diffMean, nhits = NA, ncase = NA, nalt = NA, ndrawn = NA, hits = NA, 
        test = test, stringsAsFactors = FALSE)
}
