...
getTestsModTable <- function(K2res) {
    ...
    ## Format table
    K2modTestTable <- do.call(rbind, lapply(names(K2resList),
        function(x) {
    ...
    ## Sort columns
    K2modTestTable <- K2modTestTable[, c("value", "node", "edge",
        "pval", "fdr", "stat", "df", "obsMean", "altMean", "diffMean",
        "nhits", "ncase", "nalt", "ndrawn", "hits", "test")]
