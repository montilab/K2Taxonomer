...
K2dendro <- function(K2res){
  ...
  # Add list
  aList <- list()
  newLabel <- splitNames[1]; names(newLabel) <- NULL
  ...
  for(i in bList){
    ...
    newLabel <- splitNames[i]; names(newLabel) <- NULL
    
    # Set height = 1 if terminal split, otherwise get stability and cumulative stability
