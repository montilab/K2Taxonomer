...
#' @param use Generate test statistics or use means from differential analysis
#' for clustering.
...
runK2Taxonomer <- function(eSet, cohorts = NULL, vehicle = NULL, covariates = NULL, 
    block = NULL, use = c("Z", "MEAN"), nFeats = nrow(eSet) * 0.02, featMetric = c("square", 
        "mad"), nBoots = 200, clustFunc = hclustWrapper, linkage = "mcquitty", info = NULL, 
    infoClass = NULL, genesets = NULL, qthresh = 0.05, cthresh = 0, ntotal = 20000, 
    ssGSEAalg = c("gsva", "ssgsea", "zscore", "plage"), ssGSEAminQvalue = 0.05, ssGSEAcores = 0, 
    oneoff = FALSE, stabThresh = -1, geneURL = NULL, genesetURL = NULL) {
    
    ## Match arguments
    K2res <- K2preproc(eSet, cohorts = cohorts, vehicle = vehicle, covariates = covariates, 
        block = block, infoClass = infoClass, use = use, nFeats = nFeats, featMetric = featMetric, 
        nBoots = nBoots, clustFunc = clustFunc, linkage = linkage, qthresh = qthresh, 
        cthresh = cthresh, ntotal = ntotal, ssGSEAalg = ssGSEAalg, ssGSEAminQvalue = ssGSEAminQvalue, 
