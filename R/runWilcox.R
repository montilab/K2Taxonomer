...
.runWilcox <- function(csDF, alternative = "two.sided") {
    ...
    ## Get type of test
    test <- c("one-sided Wilcox RS Test", "two-sided Wilcox RS Test")[as.numeric(alternative == 
        "two.sided") + 1]
    
    ## Generate results
    data.frame(pval = pval, stat = stat, df = NA, obsMean = obsMean, altMean = altMean, 
        diffMean = diffMean, nhits = NA, ncase = NA, nalt = NA, ndrawn = NA, hits = NA, 
        test = "1-sided Wilcox Test", stringsAsFactors = FALSE)
}
