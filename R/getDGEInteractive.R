...
getDGEInteractive <- function(K2res, 
                              maxFDR = 0.01,
                              minDiff = 1,
                              maxPval = NULL,
                              genes = NULL,
  ...
  if(sum(genesOut) > 0) {
    if(mean(genesOut) == 1) {
      ...
      warning(paste0(
        sum(genesOut), " input gene(s) not found in results table (", mean(genesOut)*100, "% Missing)\n",
  ...
  if(sum(nodesOut) > 0) {
    if(mean(nodesOut) == 1) {
      ...
      warning(paste0(
        sum(nodesOut), " input nodes(s) not found in results table (", mean(nodesOut)*100, "% Missing)\n",
  ...
  datatable(dgetab, rownames=FALSE,
